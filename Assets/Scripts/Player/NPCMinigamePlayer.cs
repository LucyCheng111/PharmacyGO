...
public class NPCMinigamePlayer : MonoBehaviour
{
    ...
    public void StartMinigame()
    {
        ...
        {
            MainCanvas.Instance.GetComponentInChildren<CardMatchingMinigame>().StartCardMatching(level);
            MainCanvas.Instance.GetComponentInChildren<CardMatchingMinigame>().RestartPlay();
        }
        else if(type == MinigameType.WordBank)
        {
            MainCanvas.Instance.GetComponentInChildren<WordBankMinigame>().StartWordBank(difficulty, level);
        }
        else if(type == MinigameType.Slapjack)
        {
            MainCanvas.Instance.GetComponentInChildren<SlapjackMinigame>().StartSlapjack(difficulty, level);
        }
    }
}
